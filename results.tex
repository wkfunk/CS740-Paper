...
In this section, we preset our design for a system, Harpocrates (Greek god of silence, secrets, and confidentidality), which allows users to easily compensate publishers at a fair rate for their content without being forced to view ads..
...
This means that solutions that involve adding significant server-side code, such as would be required to enable many of the services described in Section ~\ref{academic_related}, are out of the question.
...
Our solution relies on fact that publishers ad servers and ad exchanges are efficient marketplaces for determining the fair value for a given piece of ``real estate'' on the internet.
In order to take advantage of this fact, Harpocrates sits inline in the ``waterfall'' that currently exists in many publisher ad servers (Figure ~\ref{fig:waterfall}).
...
Hence, its implementation cost is no higher than that of adding another ad network for a given publisher and it requires no additional infrastructure or protocol that isn't already there.
